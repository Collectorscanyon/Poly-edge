...
export default function PolyEdgeScanner() {
  ...
  const { executeWithBankr, isBankrReady, loading: bankrLoading } = useBankrTrade()
  ...
  const executeViaBankr = async (market) => {
    try {
      await executeWithBankr(market, 250)
      showToast('Bankr execution sent from your Privy wallet')
    } catch (err) {
      showToast('Bankr execution failed — check console / Bankr')
    }
  }
            ...
            <div key={market.id} className="glass gradient-border relative overflow-hidden rounded-2xl p-[1px]">
              <div className="glass relative rounded-2xl p-6">
                ...
                <div className="mt-5 flex flex-col gap-3 sm:flex-row">
                  ...
                  <button
                    onClick={() => executeViaBankr(market, analysis)}
                    disabled={bankrLoading || !isBankrReady}
                    className={`flex-1 rounded-xl border px-4 py-3 text-sm font-semibold transition ${
                      isBankrReady
                        ? 'border-fuchsia-500/60 bg-gradient-to-r from-purple-600 to-pink-600 text-white hover:scale-[1.01] hover:shadow-lg hover:shadow-fuchsia-500/40'
                        : 'border-white/10 bg-white/5 text-slate-400 cursor-not-allowed opacity-60'
                    }`}
                  >
                    {bankrLoading ? 'Executing...' : isBankrReady ? 'EXECUTE WITH BANKR' : 'LOGIN WITH X FIRST'}
                  </button>
                  <button
